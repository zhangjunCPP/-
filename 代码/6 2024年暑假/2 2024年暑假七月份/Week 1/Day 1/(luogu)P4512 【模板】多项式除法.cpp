...
signed main(){
	int n,m;
	cin>>n>>m;
	for(int i=0;i<=n;i++)cin>>f[i],q[n-i]=f[i];
	for(int i=0;i<=m;i++)cin>>g[i],gr[m-i]=g[i];
	for(int i=n-m+1;i<=n;i++)q[i]=0;
	for(int i=n-m+1;i<=m;i++)gr[i]=0;
	Inv(gr,inv,n-m+1);
	mul(q,inv,n-m+1,n-m+1);
	for(int i=n-m+1;i<=2*(n-m);i++)q[i]=0;
	reverse(q,q+n-m+1);
	for(int i=0;i<=n-m;i++)cout<<q[i]<<" ";
	cout<<"\n";
	mul(q,g,n-m+1,m+1);
	for(int i=0;i<m;i++)r[i]=(f[i]-q[i]+mod)%mod;
